        ...
        <main className="flex-1 flex flex-col p-4 sm:p-6 lg:p-8 z-10 relative overflow-y-scroll">
          ...
          <div className="w-full max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="bg-white/70 backdrop-blur-2xl rounded-[32px] p-6 sm:p-8 border border-white shadow-xl flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold flex items-center gap-2 text-gray-800"><Trophy className="text-amber-500" />Loyalty Tier</h2>
                  <span className="text-xs font-semibold text-purple-600 bg-purple-50 px-3 py-1 rounded-full border border-purple-100">Elite</span>
                  </div>
                  <p className="text-gray-400 text-sm font-medium">Your current loyalty status</p>
                  <h1 className="text-5xl font-black text-gray-900 mt-2 tracking-tight">{walletAddress ? "Gold Tier" : "---"} <span className="text-xl font-bold text-gray-400">Trader</span></h1>
                  </div>
                  <div className="mt-8 pt-4 border-t border-gray-100 text-xs text-green-600 font-semibold flex items-center gap-1.5"><TrendingUp size={16} /> You are 250 PTS away from Platinum.</div>
                  </motion.div>
                  </div>
