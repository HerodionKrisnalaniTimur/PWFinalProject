              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {rewardItems.map((item) => (
                    <div key={item.id} onClick={() => openItemDetail(item)} className="bg-white rounded-[24px] p-4 flex flex-col shadow-sm border border-gray-100 group cursor-pointer hover:shadow-lg transition-shadow">
                      
                      {/* Kotak Gambar */}
                      <div className="relative w-full aspect-square rounded-[16px] overflow-hidden bg-[#F8F9FA] border border-gray-100 flex items-center justify-center">
                        <img src={item.img} alt={item.itemName} className="w-full h-full object-contain p-2 group-hover:scale-105 transition-transform duration-500" />
                        
                        {/* Tag Promo (Pindah ke kiri bawah gambar) */}
                        {item.tag && (
                          <div className="absolute bottom-3 left-3 bg-[#D43D9D] text-white text-[10px] font-bold px-3 py-1 rounded shadow-sm uppercase tracking-wider">
                            {item.tag}
                          </div>
                        )}
                      </div>
                      
                      {/* Detail Teks (Nama di Kiri, Harga di Kanan) */}
                      <div className="mt-4 mb-5 flex justify-between items-start gap-3 flex-1 px-1">
                        {/* Kiri: Nama & Studio */}
                        <div className="flex flex-col flex-1">
                          <h4 className="font-bold text-gray-800 text-[14px] leading-tight mb-1 group-hover:text-[#27BDE2] transition-colors pr-2">{item.itemName}</h4>
                      ...
                      {/* Tombol Add to Cart (Rounded penuh dengan Ikon) */}
