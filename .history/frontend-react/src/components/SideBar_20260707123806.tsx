...
import { LayoutGrid, Droplets, Star, X, Send, Disc, BookOpen, Menu, ChevronLeft, RefreshCw, Coins } from 'lucide-react';
    ...
    <>
      ...
      >
        ...
        <nav className="flex flex-col gap-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
          ...
          {/* Menu Faucet baru */}
          <Link to="/faucet" replace className={`p-4 rounded-2xl flex flex-col gap-1 transition-all ${currentPath === '/faucet' ? 'bg-white shadow-sm border border-white' : 'text-gray-400 hover:text-gray-600 hover:bg-white/50'}`}>
            <div className={`flex items-center gap-3 font-bold text-lg ${currentPath === '/faucet' ? 'text-blue-600' : ''}`}>
              <Coins size={20} /> Faucet
            </div>
            <p className={`text-xs ml-8 whitespace-nowrap ${currentPath === '/faucet' ? 'text-gray-500' : ''}`}>Claim test tokens</p>
          </Link>
        </nav>
