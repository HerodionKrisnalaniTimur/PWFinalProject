...
export const PointsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchUserActivities = async (walletAddress: string) => {
    try {
      ...
      if (response && response.success) {
        ...
        const total = dbActivities.reduce((acc: number, curr: any) => acc + curr.pointsAdded, 1250);
        setUserPoints(total);
      }
    } catch (error) {
  ...
  // Fungsi sakti untuk menambah poin & mencatat riwayat ke database & state lokal
  const addActivity = async (type: string, description: string, points: number, walletAddress?: string, chain?: string) => {
    ...
    // 2. Perbarui state lokal agar UI langsung update tanpa reload
  ...
  return (
    <PointsContext.Provider value={{ userPoints, recentActivity, addActivity, fetchUserActivities }}>
      {children}
